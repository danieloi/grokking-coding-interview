// @ts-ignore
function dfs(nums, n, subset_sum, memo) {
  if (memo[n][subset_sum] != null) {
  ...
  let result =
    dfs(nums, n - 1, subset_sum - nums[n - 1], memo) ||
    dfs(nums, n - 1, subset_sum, memo);
...
function canPartitionArray(nums) {
  ...
  const memo = new Array(n + 1)
    .fill(null)
    .map(() => new Array(subset_sum + 1).fill(null));

  return dfs(nums, n - 1, subset_sum, memo);
}
