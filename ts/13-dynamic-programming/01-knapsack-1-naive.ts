...
function findMaxKnapsackProfitHelper(
  ...
  // Recursive cases If the weight of the nth item is
  // less than capacity, then:
  if (weights[n - 1] <= capacity) {
    // We either include the item and deduct the weight
    // of the item from the knapsack capacity (to get
    // the remaining capacity) or we don't include the
    // item at all. We pick the option that yields the
    // highest value.
    return Math.max(
      values[n - 1] +
    ...
    // Item can't be added to our knapsack if its weight
...
function main() {
  ...
  // You can uncomment the lines below and check how
  // this recursive solution causes a time-out
